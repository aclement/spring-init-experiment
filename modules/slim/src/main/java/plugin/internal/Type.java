...
import plugin.internal.TypeSystem.MissingTypeException;
...
//			String tname = a.getName();//t.getName();
...
//	}
	...
	public List<Annotation> getDeclaredAnnotations(Predicate<Annotation> filter) {
		return getDeclaredAnnotations().stream().filter(filter).collect(Collectors.toList());
	}
		...
		if (annotations == null) {
			...
			if (node.visibleAnnotations != null) {
				for (AnnotationNode an: node.visibleAnnotations) {
					annotations.add(new Annotation(typeSystem, an));
	//				try {
...
//			}
	...
	public String getShortName() {
		String s = getName(); // org/springframework/boot/autoconfigure/mustache/MustacheServletWebConfiguration
