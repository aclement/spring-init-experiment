...
//			String sourceCode = ((StringBasedJavaSourceFileObject)diagnostic.getSource()).getSourceCode();
	...
	public CompilationResult compile(FileDescriptor[] sources, FileDescriptor[] resources, CompilationOptions compilationOptions, List<File> dependencies) {
		...
		options.add(new File("/Users/aclement/forks/2/spring-init-experiment/modules/processor/target/classes/").toString());
		options.add("-source");
		options.add("1.8");
...
//			String sourceCode = ((StringBasedJavaSourceFileObject)diagnostic.getSource()).getSourceCode();
		...
		if (success) {
			List<InMemoryJavaFileObject> ccds = fileManager.getOutputFiles();
			compilationResult.setCompiledClassDefinitions(ccds);
//			if (compilationOptions.shouldLoadClasses()) {
//				List<Class<?>> classes = new ArrayList<>();
//				try (SimpleClassLoader ccl = new SimpleClassLoader(this.getClass().getClassLoader())) {
//					for (CompiledClassDefinition ccd: ccds) {
//						Class<?> clazz = ccl.defineClass(ccd.getClassName(), ccd.getBytes());
