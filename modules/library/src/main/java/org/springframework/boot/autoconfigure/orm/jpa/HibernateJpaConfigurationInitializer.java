...
public class HibernateJpaConfigurationInitializer implements ApplicationContextInitializer<GenericApplicationContext> {
  ...
  public void initialize(GenericApplicationContext context) {
    ...
    if (conditions.matches(HibernateJpaConfiguration.class)) {
      if (context.getBeanFactory().getBeanNamesForType(HibernateJpaConfiguration.class).length==0) {
        context.getBeanFactory().getBean(ImportRegistrars.class).add(HibernateJpaConfiguration.class, "org.springframework.boot.context.properties.EnableConfigurationPropertiesImportSelector");
